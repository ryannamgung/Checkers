...
public final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static LinkedList<MouseClick> mouseClicked = new LinkedList<MouseClick>();
	...
	/**
	 * Returns true if the user has clicked the moust (that has not yet been
	 * processed).
	 *
	 * @return {@code true} if the user has clicked the mouse (that has not yet
	 * been processed by {@link #nextMouseClicked()}; {@code false} otherwise
	 */
	public static boolean hasNextMouseClicked() {
		synchronized (mouseLock) {
			return !mouseClicked.isEmpty();
		}
	}

	/**
	 * Returns the next click that was mouse clicked by the user (that your
	 * program has not already processed).  This method should be preceded by a
	 * call to {@link #hasNextMouseClicked()} to ensure that there is a next key
	 * to process.  This method returns an object packaging an x,y coordinate
	 * corresponding to the mouse click.
	 *
	 * @return the next mouse clicked by the user (that your program has not
	 * already processed).
	 * @throws NoSuchElementException if there is no remaining mouse click
	 */
	public static MouseClick nextMouseClicked() {
		synchronized (mouseLock) {
			if (mouseClicked.isEmpty()) {
				throw new NoSuchElementException("your program has already processed all mouse clicked");
			}
			return mouseClicked.removeLast();
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		synchronized (mouseLock) {
			mouseClicked.add(new MouseClick(mouseX, mouseY));
		}
	}
